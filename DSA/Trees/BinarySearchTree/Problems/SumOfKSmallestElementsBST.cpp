...
struct Node {
  ...
  int sum(Node* root, int k) {
        // code here
        int sum=0;
        sumHelper(root,k,sum);
        return sum;
    }
    
  void sumHelper(Node* root, int& k, int& sum) {

        // Nothing to process OR work already finished
        if (!root || k == 0)
            return;

        // LEFT
        sumHelper(root->left, k, sum);

        // Left subtree may have finished the job
        if (k == 0)
            return;
        ...
        // Current node may have finished the job
        if (k == 0)
            return;

        // RIGHT
        sumHelper(root->right, k, sum);
    }

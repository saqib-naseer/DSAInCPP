...
struct Node {
  ...
  Node(int data) {
    ...
    bool isValidBST(Node* root) {
        ...
        return validate(root,prev,isFirstVisit);
    }

     bool validate(Node* root,int& prev,bool& isFirstVisit) {
        
        if(!root){
            return 1;
        } 

        bool left = validate(root->left,prev,isFirstVisit);
        if(left==0) return 0;
    
        if(!isFirstVisit && root->val<=prev) return 0;
        prev=root->data;
        isFirstVisit=0;
        return validate(root->right,prev,isFirstVisit);
    }

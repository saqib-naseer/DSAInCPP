...
/* Approach 2 : Using Prev as TreeNode Pointer, there is not need of isFirst bit
 ...
 int minDiffInBST(Node* root) {
        ...
        minDiffInBSTHelper(root,prev,ans);
        return ans;
    }

    void minDiffInBSTHelper(Node* root,Node* &prev,int& ans){
        if(!root) return;
        minDiffInBSTHelper(root->left,prev,ans);
        
        if(prev) ans = min(ans,root->data-prev->data);

        prev = root;
        minDiffInBSTHelper(root->right,prev,ans);
    }


*/

/* Approach 1 : Using isFirst bit
     int minDiffInBST(Node* root) {
        int ans = INT_MAX;
        int prev = INT_MIN;
        bool isFirst=1;
        minDiffInBSTHelper(root,prev,ans,isFirst);
        return ans;
    }

    void minDiffInBSTHelper(Node* root,int &prev,int& ans,bool& isFirst){
        if(!root) return;
        minDiffInBSTHelper(root->left,prev,ans,isFirst);
        if(!isFirst) ans = min(ans,root->data-prev);
        isFirst=0;
        prev = root->data;
        minDiffInBSTHelper(root->right,prev,ans,isFirst);
    }
*/
int main() {
